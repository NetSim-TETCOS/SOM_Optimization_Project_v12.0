...
void fn_netsim_matlab_run(int s_count, int n_cls)
{
	...
	sprintf_s(buf, BUFSIZ,"[A,B,C]=som_optimization([Xc,Yc],%d,%d,Pwr,max_energy)", s_count, n_cls);
	status = engEvalString(ep, buf);
